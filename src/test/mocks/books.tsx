...
let id = 100;
const authors = [...Array(30)].map((val, id) => ({
  id,
  name: `Author${id}`,
  notes: `notes${id}`,
}));
const genres = [...Array(30)].map((val, id) => ({
  id,
  name: `Genre${id}`,
  notes: `notes${id}`,
}));
const types = [...Array(30)].map((val, id) => ({
  id,
  name: `Type${id}`,
  notes: `notes${id}`,
}));
...
export const books = [
  ...
  rest.get(urls.BOOKS, (req, res, context) => {
    return res(context.status(200), context.json(staticFakeBookData));
  }),

  rest.delete(urls.BOOKS + ":book/", (req, res, context) => {
    const { book } = req.params;
    const idx = staticFakeBookData.findIndex((e) => e.id === book);
    if (idx !== -1) return res(context.delay(), context.status(200));
    else
      return res(
        context.delay(),
        context.status(404),
  ...
  rest.get(urls.AUTHORS, (req, res, context) => {
    return res(context.status(200), context.json(authors));
  }),
  rest.post(urls.AUTHORS, (req, res, context) => {
    ...
    } else return res(context.status(400), context.text("Bad request"));
  }),
  rest.delete(urls.AUTHORS + ":id/", (req, res, context) => {
    ...
    return res(context.delay(), context.status(200));
  }),
  ...
  rest.post(urls.GENRES, (req, res, context) => {
    ...
    if (name && !genres.find((e) => e.name === name)) {
      ...
      return res(context.delay(), context.status(201), context.json(newGenre));
    } else return res(context.status(400), context.text("Bad request"));
  }),

  /* Literature: Types*/
  rest.get(urls.TYPES, (req, res, context) => {
    return res(context.status(200), context.json(types));
  }),
  rest.post(urls.TYPES, (req, res, context) => {
    ...
    if (name && !types.find((e) => e.name === name)) {
      ...
      types.push(newType);
      return res(context.delay(), context.status(201), context.json(newType));
    } else return res(context.status(400), context.text("Bad request"));
  }),
];
